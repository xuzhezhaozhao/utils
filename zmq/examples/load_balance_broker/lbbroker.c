/*******************************************************
 @Author: zhezhaoxu
 @Created Time : Thu 05 Oct 2017 04:38:50 PM CST
 @File Name: lbbroker.c
 @Description: p98
 负载均衡到消息代理，REQ 和 ROUTER 帧处理
 ******************************************************/
...
int main(void) {
	...
	for (client_nbr = 0; client_nbr < NBR_CLIENTS; client_nbr++) {
		pthread_t client;
		pthread_create(&client, NULL, client_task,
					   (void *)(intptr_t)client_nbr);
	}
	int worker_nbr;
	...
	while (1) {
		...
		if (items[0].revents & ZMQ_POLLIN) {
			...
			if (strcmp(client_id, "READY") != 0) {
				...
				free(reply);
				if (--client_nbr == 0) break;  //  Exit after N messages
			}
			free(client_id);
		}
